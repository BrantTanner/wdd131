...
function search(event) {
    ...
    const recipeQuery = document.querySelector('#search-input').value.toLowerCase();

    //Filter recipes
    const filterRecipes = recipes.filter(recipe => {
        return (
            recipe.image.toLowerCase().includes(recipeQuery) ||
    ...
    // Display results
    filterRecipes.forEach(recipe => renderRecipe(recipe));
}
...
function renderRating(element, rating) {
    ...
    element.setAttribute("aria-label", `Rating: ${rating} out of ${maxStars} stars`);
    element.innerHTML = "";
    ...
    for (let i = 1; i <= maxStars; i++) {
        ...
        if (i <= Math.floor(rating)) {
            ...
            star.textContent = "⯪"; // half star (simple fallback)
        } else {
...
// template to dynamically load in recipes
function recipesTemplate(recipe) {
    return `<div class="recipe-card">
        <img src="${recipe.image}" alt="${recipe.name} picture" width="500px" height="auto">
        
        <div class="recipe-content">
            <button type="button">${recipe.tags.join(", ")}</button>
            <h2>${recipe.name}</h2>
...
// render in single recipe 
function renderRecipe(recipe) {
    // Insert HTML
    recipeContainer.insertAdjacentHTML("beforeend", recipesTemplate(recipe));

    // Get the last added recipe card
    const newCard = recipeContainer.querySelector(".recipe-card:last-child");

    // Find its rating element
    const ratingElement = newCard.querySelector(".rating");

    // Render rating stars
    renderRating(ratingElement, recipe.rating);
}
...
// Page init show 1 random recipe
function init() {
